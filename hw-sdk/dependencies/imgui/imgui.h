...
namespace ImGui
{
	...
	IMGUI_API bool          Checkbox( const char* label, bool* v, int x_pos = 5 );
	...
	// - Adjust format string to decorate the value with a prefix, a suffix, or adapt the editing and display precision e.g. "%.3f" -> 1.234; "%5.2f secs" -> 01.23 secs; "Biscuit: %.0f" -> Biscuit: 1; etc.
	IMGUI_API bool          SliderFloat( const char* label, float* v, float v_min, float v_max, const char* format = "%.3f", float power = 1.0f, int same_line_pos = -3 );     // adjust format to decorate the value with a prefix or a suffix for in-slider labels or unit display. Use power!=1.0 for power curve sliders
	...
	IMGUI_API bool          Keybind( const char* id, int* current_key, int sameline = 5 );
	IMGUI_API void RenderTab( const char* label, int tab_number );
	// Popups, Modals
